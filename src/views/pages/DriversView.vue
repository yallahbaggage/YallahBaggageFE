<template>
  <div class="app-management-page">
    <BaseHeader
      :pageImage="UserIcon"
      :title="t('drivers')"
      :desc="t('manageAndTrackYourDrivers')"
      :show-button="true"
      :button-text="t('newDriver')"
      v-on:button-pressed="() => (isDriverDrawerOpen = true)"
    />
    <div class="page-content">
      ...
      <div v-else>
        <div class="cards">
          <InfoCard class="infoCard" :cardTitle="t('totalDrivers')">
            {{ stats.totalWorkers }}
          </InfoCard>
          <InfoCard class="infoCard" :cardTitle="t('driversOnTransfer')">
            {{ stats.workersWithTransfers }}
          </InfoCard>
          <InfoCard class="infoCard" :cardTitle="t('availableDrivers')">
            ...
            <form @submit.prevent="onAddButtonPressed()" class="drawer-form">
              ...
              <div class="drawer-form-group">
                ...
                <input
                  ...
                  type="text"
                  class="form-input"
                  placeholder="Driver full name here"
              ...
              <div class="action-btns">
                <ActionButton
                  ...
                  "
                />
                <ActionButton class="action-Btn" :buttonText="t('addDriver')" buttonType="submit" />
              </div>
            </form>
          </div>
        </Drawer>
        ...
        <!-- delete Drawer -->
        <Drawer
          :isOpen="isDeleteDriverDrawerOpen"
          :desc="t('driver') + ' ' + '#' + selectedWorker?._id.substring(0, 10)"
          :title="selectedWorker?.name"
          :drawerStatus="selectedWorker?.status ? selectedWorker?.status : 'available'"
          @close="isDeleteDriverDrawerOpen = false"
        >
          <div style="max-height: 75vh;">
            <form class="form">
              <div>
                ...
                <div>
                  ...
                  <div class="drawer-info">
                    <p class="drawer-key">{{ t('driverID') }}</p>
                ...
                <div class="action-btns">
                  <ActionButton
                    ...
                    buttonColor="white"
                    class="action-Btn"
                    @button-pressed="() => (isDeleteDriverDrawerOpen = false)"
                  />
                  <ActionButton
                    button-color="error"
                    :buttonText="t('deleteDriver')"
                    class="action-Btn"
                    @button-pressed="
                      () => {
                        isConfirmDeletePopupVisible = true
                        isDeleteDriverDrawerOpen = false
                      }
                    "
                  />
                </div>
        ...
        <!-- update Drawer -->
        <Drawer
          :isOpen="isUpdateDriverDrawerOpen"
          :desc="t('driver') + ' ' + '#' + selectedWorker?._id.substring(0, 10)"
          :title="selectedWorker?.name"
          :drawerStatus="selectedWorker?.status ? selectedWorker?.status : 'available'"
          @close="isUpdateDriverDrawerOpen = false"
        >
          <div style="max-height: 75vh;">
                ...
                <div>
                  ...
                  <div class="action-btns">
                    <ActionButton
                      ...
                      buttonColor="white"
                      class="action-Btn"
                      @button-pressed="() => (isUpdateDriverDrawerOpen = false)"
                    />
        ...
        <ConfirmPopupDialog
          ...
          :title="t('deleteDriverConfirmTitle')"
          :message="t('deleteDriverConfirmDescription')"
...
const { t } = useI18n()
const isDriverDrawerOpen = ref(false)
const isDeleteDriverDrawerOpen = ref(false)
const isUpdateDriverDrawerOpen = ref(false)
...
const headers = [
  ...
  { title: t('driver'), key: 'name', sortable: false },
...
const onDeleteButtonPressed = async (selectedWorkerId: string) => {
  ...
  toastDeleteMessage(t('toastDeleteDriverTitle'), t('toastDeleteDriverDescription'))
  isDeleteDriverDrawerOpen.value = false
...
const onAddButtonPressed = async () => {
  try {
    ...
    isDriverDrawerOpen.value = false
    toastSuccessMessage(t('toastAddDriverTitle'), t('toastAddDriverDescription'))
...
function viewDetails(item: any) {
  ...
  isUpdateDriverDrawerOpen.value = true
}
...
function deleteEmpeloyee(item: any) {
  selectedWorker.value = item as IWorker
  isDeleteDriverDrawerOpen.value = true
}
...
const onUpdateButtonPressed = async () => {
  try {
    ...
    isUpdateDriverDrawerOpen.value = false
    await fetchWorkers()
    toastSuccessMessage(t('toastUpdateDriverTitle'), t('toastUpdateDriverDescription'))
  } catch (error) {
    toastErrorMessage(
      t('anErrorOccured'),
      typeof error === 'string'
